...
public class EntryPanel extends JPanel {
    ...
    public EntryPanel() {
        ...
        String[] types = {"Car", "Motorcycle", "SUV or Truck", "Handicapped", "Reserved"};
        typeCombo = new JComboBox<>(types);
        topPanel.add(typeCombo);
        
        // Add a "Help" label
        JLabel helpLabel = new JLabel("(Step 2: Click a GREEN spot below to park)");
    ...
    private void refreshGrid() {
        ...
        // CHANGE: Loop from Floor 5 down to 1 (Descending)
        // This puts Floor 5 at the top of the screen
    ...
    private JButton createInteractionButton(ParkingSpot spot) {
        ...
        if (spot.isOccupied()) {
            ...
            btn.setBackground(new Color(144, 238, 144)); // Available spots are Green!
    ...
    private void handleSpotClick(ParkingSpot spot) {
        ...
        // 3. Normal Parking Logic
        String selectedType = (String) typeCombo.getSelectedItem();
        Vehicle v;

        // Check what the user selected to create the right object
        if ("Motorcycle".equalsIgnoreCase(selectedType)) {
            v = new model.Motorcycle(plate);
        } else {
            // Default to Car for "Car", "SUV", "Reserved", "Handicapped"
            v = new model.Car(plate);
        }

        boolean success = ParkingService.getInstance().parkVehicleAt(v, spot.getSpotId());
        
        if (success) {
            JOptionPane.showMessageDialog(this, "Parked Successfully!");
            
            // Clear inputs so they can't double-submit
            plateField.setText("");
